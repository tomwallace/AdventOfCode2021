...
public class DaySeven : IAdventProblemSet
{
    ...
    public string PartA()
    {
        var fuelCosts = FindFuelCostsForAllPositions(ActualInput, false);
        var cheapest = fuelCosts.Min(f => f.Value);
    ...
    public string PartB()
    {
        var fuelCosts = FindFuelCostsForAllPositions(ActualInput, true);
        var cheapest = fuelCosts.Min(f => f.Value);
        return cheapest.ToString();
    }

    public Dictionary<int, int> FindFuelCostsForAllPositions(List<int> input, bool applyFuelStepIncrease)
    {
        var crabSubs = input.Select(i => new CrabSub(i)).ToList();
        var min = crabSubs.Min(c => c.Location);
        var max = crabSubs.Max(c => c.Location);

        var fuelCosts = new Dictionary<int, int>();
        for (var targetLocation = min; targetLocation <= max; targetLocation++)
        {
            fuelCosts.Add(targetLocation, DetermineFuelCostForAllCrabSubs(targetLocation, crabSubs, applyFuelStepIncrease));
        }

        return fuelCosts;
    }

    private int DetermineFuelCostForAllCrabSubs(int targetLocation, List<CrabSub> crabSubs, bool applyFuelStepIncrease)
    {
        return crabSubs.Sum(c => c.MoveCostInFuel(targetLocation, applyFuelStepIncrease));
    }
}